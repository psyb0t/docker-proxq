...
func TestParse_Defaults(t *testing.T) {
	...
	assert.Equal(t, "", cfg.PathFilter)
	assert.Equal(t, proxy.PathFilterModeBlacklist, cfg.PathFilterMode)
...
func TestParse_EnvOverrides(t *testing.T) {
	...
	t.Setenv("PROXQ_DIRECT_PROXY_MODE", proxy.DirectProxyModeRedirect)
	t.Setenv("PROXQ_PATH_FILTER", "^/uploads,^/ws")
	t.Setenv("PROXQ_PATH_FILTER_MODE", proxy.PathFilterModeWhitelist)
	...
	assert.Equal(t, "^/uploads,^/ws", cfg.PathFilter)
	assert.Equal(t, proxy.PathFilterModeWhitelist, cfg.PathFilterMode)
